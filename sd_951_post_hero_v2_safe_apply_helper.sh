...
SD_ID="sd_951_post_hero_v2_safe"
ROOT="$(pwd)"
...
# Auto-restore PostCard if we detect the broken v1 marker
if [[ -f "$POSTCARD" ]] && grep -q "sd_951_post_hero_detail_style" "$POSTCARD" 2>/dev/null; then
  echo "⚠️ Detected broken sd_951 v1 marker in PostCard. Attempting auto-restore from v1 backup..."
  CAND="$(ls -dt .backup_sd_951_post_hero_v1_* 2>/dev/null | head -n 1 || true)"
  if [[ -n "$CAND" ]] && [[ -f "$CAND/PostCard.tsx.bak" ]]; then
    cp -a "$CAND/PostCard.tsx.bak" "$POSTCARD"
    echo "✅ Restored PostCard from: $CAND/PostCard.tsx.bak"
  else
    echo "⚠️ Could not find a v1 backup with PostCard.tsx.bak."
    echo "   If typecheck fails, run:"
    echo "     git restore frontend/src/components/PostCard.tsx"
  fi
fi
...
// sd_951_v2: Dedicated "hero" renderer for Post Detail root post.
// Avoids risky edits to PostCard.tsx while we iterate on thread UI.
...
if (!page.includes(MARK)) {
  ...
  const reRoot = /<PostCard([\s\S]*?)post=\{found\.post\}([\s\S]*?)\/>/m;
  must(reRoot.test(page), "sd_951_v2: could not find <PostCard ... post={found.post} ... /> in PostDetail page.");
  page = page.replace(reRoot, () => {
    return `<PostHero post={found.post} side={found.side} theme={theme} onReply={() => { try { replyInputRef.current?.focus(); } catch {} }} onMore={() => { try {} catch {} }} />`;
  });
...
try {
  if (fs.existsSync(STATE)) {
    const mark = "**sd_951_v2:** Thread: root post hero component (PostHero) — clean detail layout without risky PostCard surgery.";
