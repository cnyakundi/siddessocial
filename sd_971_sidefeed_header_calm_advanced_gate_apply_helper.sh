...
SD_ID="sd_971_sidefeed_header_calm_advanced_gate"
...
MARK = "sd_971_sidefeed_header_calm_advanced_gate"

# 1) Add advanced flag after the first `const <var> = useSearchParams();`
if " const advanced " not in s and "const advanced " not in s:
    m = re.search(r'^\s*const\s+(\w+)\s*=\s*useSearchParams\(\);\s*$', s, flags=re.M)
    if not m:
        raise SystemExit("ERROR: Could not find `const <var> = useSearchParams();` in SideFeed.tsx")
    var = m.group(1)
    insert = m.group(0) + "\n  const advanced = (" + var + '.get("advanced") || "").trim() === "1"; // ' + MARK + "\n"
    s = s[:m.start()] + insert + s[m.end():]

# 2) Gate DEV badge behind advanced
# from: process.env.NODE_ENV !== "production" ? (
# to:   process.env.NODE_ENV !== "production" && advanced ? (
if 'process.env.NODE_ENV !== "production" ? (' in s:
    s = s.replace('process.env.NODE_ENV !== "production" ? (', 'process.env.NODE_ENV !== "production" && advanced ? (', 1)

# 3) Ensure CircleFilterBar gets currentSide={side} (helps sides with 0 circles)
if "CircleFilterBar" in s and "currentSide={side}" not in s:
    s = s.replace("activeSet={activeSet}\n", "activeSet={activeSet}\n      currentSide={side}\n", 1)

# 4) Circle “New” behavior:
...
# 5) Public Tune button:
# Topics normal; Trust+Counts only when advanced
s = s.replace(
...
# 6) PublicTuneSheet props: Trust/Counts advanced-only
s = s.replace(
...
if [[ -f "$STATE" ]] && ! grep -q "$SD_ID" "$STATE"; then
  printf "\n- **%s:** SideFeed header: gate DEV badge + dev import sheet behind ?advanced=1; Public Tune shows Topics normally; Trust/Counts only in advanced mode.\n" "$SD_ID" >> "$STATE"
fi
