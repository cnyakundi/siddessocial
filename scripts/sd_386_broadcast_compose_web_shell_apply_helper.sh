...
# sd_386_broadcast_compose_web_shell_apply_helper.sh
# Align Broadcast compose with Web Composer safety + shell:
# - modal overlay w/ safety header (Public + Broadcast)
# - char counter (800) + disable over limit
# - never drop text on failure; save draft on close/failure
# - offline -> queue + undo
# - friendly errors for trust gates + broadcast permissions
...
BK="$ROOT/.backup_sd_386_broadcast_compose_web_shell_${STAMP}"
mkdir -p "$BK"
...
echo "== sd_386: Broadcast compose Web Composer shell =="
echo "Backups: $BK"

backup_file "frontend/src/app/siddes-broadcasts/[id]/compose/page.tsx"

echo "Writing Broadcast compose page..."
cat > "$ROOT/frontend/src/app/siddes-broadcasts/[id]/compose/page.tsx" <<'EOF'
"use client";
  ...
  if (hydrated && side !== "public") {
    return (
      <>
        ...
        <PublicEnterConfirmSheet open={true} fromSide={side} onCancel={() => router.replace("/siddes-feed")} onConfirm={() => setSide("public")} />
      </>
    );
  }
...
echo "Writing docs/COMPOSER_BROADCAST_COMPOSE_V2.md ..."
mkdir -p "$ROOT/docs"
cat > "$ROOT/docs/COMPOSER_BROADCAST_COMPOSE_V2.md" <<'DOC'
...
echo ""
echo "Verify:"
echo "  - Open /siddes-broadcasts/<id>/compose"
echo "  - Over 800 chars disables Post"
echo "  - Offline queues + undo"
echo "  - Failure keeps text + inline error"
echo ""
echo "Rollback:"
echo "  cp \"$BK/frontend/src/app/siddes-broadcasts/[id]/compose/page.tsx\" \"$ROOT/frontend/src/app/siddes-broadcasts/[id]/compose/page.tsx\""
