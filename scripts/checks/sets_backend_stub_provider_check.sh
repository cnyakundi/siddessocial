...
echo "== Check: Sets backend_stub provider wiring =="
...
grep -q "/api/sets" frontend/src/lib/setsProviders/backendStub.ts && echo "✅ backendStubProvider calls /api/sets" || (echo "❌ backendStubProvider not calling /api/sets" && exit 1)
