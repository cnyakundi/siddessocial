...
if grep -q "sd_viewer" frontend/src/app/api/sets/route.ts   frontend/src/app/api/sets/[id]/route.ts   frontend/src/app/api/sets/[id]/events/route.ts; then
  echo "✅ Sets routes reference sd_viewer gate"
else
  echo "❌ Sets routes missing sd_viewer gate"
  exit 1
fi
