...
echo "== Check: Circles Django router + app install =="
...
grep -q 'PATCH' backend/siddes_backend/middleware.py && echo "✅ Dev CORS allows PATCH" || (echo "❌ Dev CORS missing PATCH (needed for Circles update)" && exit 1)

# Marker: keep state history honest
if grep -q "sd_134" docs/STATE.md; then
  echo "✅ STATE doc still includes sd_134 Circles ladder"
else
  echo "❌ docs/STATE.md missing Circles ladder marker sd_134"
  exit 1
fi
