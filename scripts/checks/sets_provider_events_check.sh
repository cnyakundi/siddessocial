...
echo "== Check: Sets provider get/update/events + local history =="
...
grep -q "export type SetEvent" frontend/src/lib/setEvents.ts && echo "✅ SetEvent type" || (echo "❌ SetEvent type missing" && exit 1)
...
grep -q "appendSetEvent" frontend/src/lib/setsProviders/local.ts && echo "✅ local provider appends events" || (echo "❌ local provider not recording events" && exit 1)
...
grep -q "/api/sets/" frontend/src/lib/setsProviders/backendStub.ts && echo "✅ backend_stub provider calls /api/sets/*" || (echo "❌ backend_stub provider not calling /api/sets/*" && exit 1)
