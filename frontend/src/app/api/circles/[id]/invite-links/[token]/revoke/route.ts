...
// POST /api/circles/:id/invite-links/:token/revoke -> Django POST /api/circles/:id/invite-links/:token/revoke
export async function POST(req: Request, ctx: { params: { id: string; token: string } }) {
  ...
  const out = await proxyJson(req, `/api/circles/${id}/invite-links/${token}/revoke`, "POST", {});
