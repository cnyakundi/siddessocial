...
import { SuggestedCirclesTray } from "@/src/components/SuggestedCirclesTray";
import { CreateCircleSheet } from "@/src/components/CreateCircleSheet";
import { CirclesJoinedPill } from "@/src/components/CirclesJoinedBanner";
...
import { onCirclesChanged } from "@/src/lib/circlesSignals";
...
function SiddesCirclesPageInner() {
  // sd_464c: restore scroll when returning to Circles list
  useReturnScrollRestore();
  const sp = useSearchParams();
  const { side: ctxSide } = useSide();

  // sd_543a: MVP default — keep Circles calm; unlock power tools via ?advanced=1
  ...
  const [items, setItems] = useState<CircleDef[]>([]);
  ...
  const [newColor, setNewColor] = useState<CircleColor>("emerald");
  ...
  const create = async (label: string, membersRaw: string, side: SideId, color?: CircleColor) => {
  ...
  <div className="mb-3 p-3 rounded-2xl border border-slate-200 bg-slate-50 text-slate-800 text-sm">
    ...
    <div className="text-xs leading-relaxed text-slate-600">
      <span className="font-bold">Create disabled (read-only)</span> — only the owner can create or edit circles in this view.
        ...
        {advanced ? (
          <div className="mb-4">
            <SuggestedCirclesTray onCreated={() => void refresh()} />
          </div>
        ) : null}
        ...
        <div className="space-y-2">
          {loading ? (
            ...
            filtered.map((s) => {
              ...
              const setTheme = getCircleTheme(s.color);
                    ...
                    <div className="min-w-0 flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {joined ? <CirclesJoinedPill className="shrink-0" /> : null}
                        </div>
...
export default function SiddesCirclesPage() {
  return (
    <Suspense
      fallback={
