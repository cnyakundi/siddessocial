...
import { emitCirclesChanged, onCirclesChanged } from "@/src/lib/circlesSignals";
import { toast } from "@/src/lib/toast";
import { getStubViewerCookie, isStubMe } from "@/src/lib/stubViewerClient";
import { CirclesJoinedBanner } from "@/src/components/CirclesJoinedBanner";
import { AddPeopleSheet } from "@/src/components/AddPeopleSheet";
import { CircleHubMoreSheet } from "@/src/components/CircleHubMoreSheet";
...
const COLOR_OPTIONS: CircleColor[] = ["orange", "purple", "emerald", "rose", "slate"];
...
function kindLabel(k: CircleEvent["kind"]) {
  switch (k) {
    case "created":
      return "Created";
    case "renamed":
...
function kindDetail(e: CircleEvent): string {
  const d: any = e.data || {};
...
export default function SiddesCircleHubPage({ params }: { params: { id: string } }) {
  ...
  const [item, setItem] = useState<CircleDef | null>(null);
  ...
  const themeSet = item ? getCircleTheme(item.color) : getCircleTheme(color);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (got) {
        ...
        // Outgoing invites for this Circle (best-effort).
        try {
  ...
  const saveSettings = async () => {
    if (!item) return;
    if (!canWrite) {
      toast.error("Only the Circle owner can change settings.");
      return;
    }
  ...
  const leaveSet = async () => {
    ...
    try {
      const res = await fetch(`/api/circles/${encodeURIComponent(item.id)}/leave`, { method: "POST" });
  ...
  const deleteSet = async () => {
    ...
    try {
      const res = await fetch(`/api/circles/${encodeURIComponent(item.id)}`, { method: "DELETE" });
      const data = await safeJson(res);
      if (!res.ok) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="px-4 py-4">
        ...
        {/* Tab body */}
        <div className="mt-3">
          {!isOwner && viewer ? <CirclesJoinedBanner viewer={String(viewer)} /> : null}
          {/* FEED */}
          {tab === "feed" ? (
...
{feedErr ? (
              ...
              {!feedLoading && !posts.length && item ? (
                <div className="p-8 rounded-2xl bg-white border border-dashed border-gray-200 text-center">
                  ...
                  <div className="text-sm text-gray-500 mb-4">Start the first post in this Circle.</div>
                  <Link
                    href={composeHref}
                    ...
                    <div className="text-xs text-gray-500 mt-1 flex items-start gap-2">
                      ...
                      <span>
                        {item?.side === "public"
                          ...
                          : `This Circle lives in ${SIDES[item ? item.side : side].label}. Only Set members can see posts here. Other Sides can't see this Circle.`}
                      </span>
                    </div>
              ...
              <div className="p-4 rounded-2xl bg-white border border-gray-200">
                ...
                {item?.members?.length ? (
                  ...
                  <div className="p-6 rounded-2xl border border-dashed border-gray-200 text-center">
                    ...
                    <div className="text-sm text-gray-500">Invite people to build this Circle.</div>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="p-4 rounded-2xl bg-white border border-gray-200">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="text-xs text-gray-500">Invite more people to this Circle</div>
                  </div>
                  <button
                    type="button"
                    disabled={!item || !canWrite}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  ...
                  <div>
                    ...
                    <select
                      value={color}
                      disabled={!canWrite}
                      onChange={(e) => setColor(e.target.value as CircleColor)}
                      className={cn(
                        "w-full px-3 py-2 rounded-xl border border-gray-200 bg-white text-sm font-semibold outline-none focus:ring-2 ring-gray-900/10",
                        !isOwner && "bg-gray-50 text-gray-600 cursor-not-allowed"
                      )}
              ...
              <div className="p-4 rounded-2xl bg-white border border-gray-200">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="text-xs text-gray-500">Server-truth event log for this Circle</div>
                  </div>
                ...
                {!events.length ? (
                  <div className="p-6 rounded-2xl border border-dashed border-gray-200 text-center">
                    ...
                    <div className="text-sm text-gray-500">
                      {isOwner ? "Create, invite, or edit the Set to generate history." : "History is generated when the owner updates this Circle."}
                    </div>
                  </div>
                ) : null}
              </div>
