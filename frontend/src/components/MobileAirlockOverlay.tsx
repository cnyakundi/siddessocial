...
/**
 * sd_496: Mobile-only adaptive latency overlay.
 * - Private ↔ Private: 150ms subtle tint
 ...
 */
export function MobileAirlockOverlay() {
  const { side } = useSide();
  ...
  const sub = enteringPublic ? `Entering ${toLabel}` : leavingPublic ? `Leaving ${fromLabel}` : `Switching to ${toLabel}`;

  return (
    <div className="fixed inset-0 z-[200] pointer-events-none lg:hidden">
      {/* Subtle tint (always) */}
      <div
        className={cn(
          "absolute inset-0",
          ...
          st.mode === "cinematic" ? "opacity-10 animate-pulse" : "opacity-5"
        )}
        aria-hidden="true"
      />
