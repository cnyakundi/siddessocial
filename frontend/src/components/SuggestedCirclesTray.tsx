...
import { SuggestedCirclesSheet } from "@/src/components/SuggestedCirclesSheet";
...
import { loadSuggestedCirclesCache, saveSuggestedCirclesCache } from "@/src/lib/localIntelligence/localSuggestedCirclesCache";
import {
...
export function SuggestedCirclesTray({ onCreated }: { onCreated?: () => void }) {
  ...
  useEffect(() => {
    const cached = loadSuggestedCirclesCache(viewerKey);
    const filtered = cached.filter((s: { id: string }) => !isSuggestionSuppressed(viewerKey, s.id));
  ...
  function persist(next: SuggestedCircle[]) {
    const clean = dedupeById(next);
    setSuggestions(clean);
    saveSuggestedCirclesCache(viewerKey, clean);
  }

  function armUndo(setIds: string[], ss: SuggestedCircle[]) {
    const ids = (setIds || []).map((x) => String(x || "").trim()).filter(Boolean);
    if (!ids.length) return;

    const token = `${Date.now()}_${Math.random().toString(16).slice(2)}`;
    const message = ids.length === 1 ? "Circle created." : `${ids.length} circles created.`;
  ...
  async function createOne(s: SuggestedCircle): Promise<string> {
  ...
  async function createMany(ss: SuggestedCircle[]): Promise<string[]> {
  ...
  async function acceptOne(s: SuggestedCircle) {
    if (busy) return;
    setBusy(true);
    try {
  ...
  async function acceptMany(ss: SuggestedCircle[]) {
    if (busy) return;
    setBusy(true);
    try {
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <div className="font-extrabold text-gray-900">Suggested Circles</div>
            </div>
