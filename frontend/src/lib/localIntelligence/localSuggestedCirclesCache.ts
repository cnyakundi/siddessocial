...
const KEY_PREFIX = "siddes.suggested_circles.cache.v1";
...
function safeParse(raw: string | null): CachePayload | null {
  ...
  try {
    ...
    const ts = typeof (j as any).ts === "number" ? (j as any).ts : 0;
    const suggestions = Array.isArray((j as any).suggestions) ? (j as any).suggestions : [];
    return { ts, suggestions } as any;
  } catch {
    return null;
  }
}
...
function dedupeById(items: SuggestedCircle[]): SuggestedCircle[] {
  ...
  for (const it of items) {
    ...
    // Normalize shape so UI never explodes on weird cache contents
    const label = String((it as any)?.label || "").trim();
    const reason = String((it as any)?.reason || "").trim();
    const members = Array.isArray((it as any)?.members)
      ? (it as any).members.map((m: any) => String(m || "").trim()).filter(Boolean)
      : [];
...
export function loadSuggestedCirclesCache(viewerKey: string, maxAgeDays = 30): SuggestedCircle[] {
  ...
  try {
    ...
    return dedupeById(payload.suggestions as any);
  } catch {
    return [];
  }
}
