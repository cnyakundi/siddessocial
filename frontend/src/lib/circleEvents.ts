...
export type CircleEventKind = "created" | "renamed" | "members_updated" | "moved_side" | "recolored";
...
function isKind(x: any): x is CircleEventKind {
...
export function loadCircleEvents(setId: CircleId): CircleEvent[] {
  if (!hasWindow()) return [];
  const map = loadMap();
  const arr = Array.isArray(map[setId]) ? map[setId] : [];
  const out = arr.map(coerceEvent).filter(Boolean) as CircleEvent[];
  out.sort((a, b) => b.ts - a.ts);
  return out;
}

export function appendCircleEvent(
  setId: CircleId,
  kind: CircleEventKind,
  by: string,
  data?: Record<string, any>,
...
export function ensureCreatedEvent(set: CircleDef, by = "me") {
  if (!hasWindow()) return;
  const existing = loadCircleEvents(set.id);
  if (existing.some((e) => e.kind === "created")) return;
  appendCircleEvent(set.id, "created", by, { label: set.label, side: set.side });
}
