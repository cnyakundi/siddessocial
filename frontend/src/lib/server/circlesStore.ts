...
export type UpdateCirclePatch = Partial<Pick<StoredSet, "side" | "label" | "members" | "color">>;

type ViewerState = {
  sets: StoredSet[];
  events: StoredCircleEvent[];
};
...
function pickCircleColor(label: string): CircleColor {
...
function seedDefaults(viewerId: string): ViewerState {
  ...
  const events: StoredCircleEvent[] = defaults.map((s) => ({
    id: newEventId(),
    setId: s.id,
    kind: "created",
    ts: s.createdAt,
...
export function listCircleEvents(viewerId: string, setId: string): StoredCircleEvent[] {
  const st = ensureViewer(viewerId);
  return st.events.filter((e) => e.setId === setId).sort((a, b) => b.ts - a.ts);
}

export function createCircle(viewerId: string, input: CreateCircleInput): StoredSet {
  const st = ensureViewer(viewerId);
  const t = now();
  ...
  const item: StoredSet = {
    ...
    side: input.side,
    label: input.label,
    color: input.color || pickCircleColor(input.label),
...
export function bulkCreateSets(viewerId: string, inputs: CreateCircleInput[]): StoredSet[] {
  const made: StoredSet[] = [];
  for (const input of inputs) {
    made.push(createCircle(viewerId, input));
  }
  return made;
}

export function updateSet(viewerId: string, id: string, patch: UpdateCirclePatch): StoredSet | null {
  const st = ensureViewer(viewerId);
  ...
  if (typeof patch.color === "string" && patch.color !== item.color) {
    ...
    item.color = patch.color as CircleColor;
    st.events.push({
      id: newEventId(),
      setId: id,
      kind: "recolored",
