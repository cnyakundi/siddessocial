...
class SetsApiSmokeTests(APITestCase):
    ...
    def test_create_requires_viewer(self):
        r = self.client.post("/api/sets", {"side": "friends", "label": "A"}, format="json")
    ...
    def test_owner_can_create_and_read(self):
        ...
        r2 = self.client.get(f"/api/sets/{sid}", HTTP_X_SD_VIEWER="me")
    ...
    def test_non_member_get_is_default_safe(self):
        ...
        r2 = self.client.get(f"/api/sets/{sid}", HTTP_X_SD_VIEWER="close")
        assert r2.status_code == 200
        d2 = r2.json()
        assert d2.get("ok") is True
    ...
    def test_member_can_read_set(self):
        ...
        r2 = self.client.get(f"/api/sets/{sid}", HTTP_X_SD_VIEWER="@a")
        assert r2.status_code == 200
        d2 = r2.json()
        assert d2.get("ok") is True
        assert d2.get("restricted") is False
