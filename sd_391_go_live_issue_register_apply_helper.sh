...
SD_ID="sd_391"
SD_TITLE="Go-live Issue Register (all identified gaps + fixes + verification)"

ROOT_DIR="$(pwd)"

if [ ! -d "${ROOT_DIR}/frontend" ] || [ ! -d "${ROOT_DIR}/backend" ]; then
  echo "ERROR: Run this from repo root (must contain frontend/ and backend/)."
  echo "Current: ${ROOT_DIR}"
  exit 1
fi

TS="$(date +%Y%m%d_%H%M%S)"
BACKUP_DIR="${ROOT_DIR}/.backup_${SD_ID}_issue_register_${TS}"
mkdir -p "${BACKUP_DIR}"
...
DOC_PATH="${ROOT_DIR}/docs/GO_LIVE_ISSUE_REGISTER.md"
...
# Link the new doc from docs/README.md (if it exists)
if [ -f "${ROOT_DIR}/docs/README.md" ]; then
  if ! grep -qF "GO_LIVE_ISSUE_REGISTER.md" "${ROOT_DIR}/docs/README.md"; then
    append_if_missing "${ROOT_DIR}/docs/README.md" "GO_LIVE_ISSUE_REGISTER.md" <<'MD'
## Go-live
...
echo
echo "OK: ${SD_ID} applied."
echo "Backups saved to: ${BACKUP_DIR}"
echo "Wrote: docs/GO_LIVE_ISSUE_REGISTER.md"
echo
echo "Next (VS Code terminal):"
echo "  open docs/GO_LIVE_ISSUE_REGISTER.md"
