# FIX: PWA — Bottom Nav Alerts (swap out Circles) + Unread Badge + Close icon clarity
...
- Replace the **Circles** tab in the bottom nav with **Alerts**.
...
## Notes
- Circles are still available on mobile via **Account** (`/siddes-profile/account`) and by direct route (`/siddes-circles`).
...
## Smoke test
1) **PWA/mobile:** confirm bottom nav shows **Alerts** (bell) instead of Circles.
